...
class WorkflowViewSet(GetSerializerMixin,
    ...
    def submit(self, request, *args, **kwargs):
        ...
        workflow = request.data['workflow_payload']
            ...
            logger.error('Workflow submission failed: '+resp.reason)
            self.set_status(resp.status_code)
            self.write('Workflow submission failed: '+resp.reason)
            self.write_error('Workflow submission failed: '+resp.reason)
            self.flush()
            return
